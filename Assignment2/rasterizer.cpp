...
{
    Vector3f p(x, y, 0.0f);
    auto edge = [](const Vector3f &a, const Vector3f &b, const Vector3f &c) {
        return (b.x() - a.x()) * (c.y() - a.y()) - (b.y() - a.y()) * (c.x() - a.x());
    };

    float c0 = edge(_v[0], _v[1], p);
    float c1 = edge(_v[1], _v[2], p);
    float c2 = edge(_v[2], _v[0], p);
...
rst::rasterizer::rasterizer(int w, int h) : width(w), height(h)
{
    ...
    if (Enable2xSSAA) {
        depth_buf_2xSSAA.resize(w * h * 4, std::numeric_limits<float>::infinity());
...
int rst::rasterizer::get_index_2xSSAA(int x, int y, int subpixel_index)
{
    return (height - 1 - y) * width * 4 + x * 4 + subpixel_index;
}
